import { Strategy, type StrategyOptions, type VerifyFunction } from 'passport-facebook';
...
export default class FacebookStrategy extends Passport implements IStrategy<Strategy> {
    ...
    verify: VerifyFunction = (accessToken, refreshToken, profile, done) => {
        this.verifyHandle<IAuthResultData>(
            async () => await this.authService.loginUserBySocials({provider: 'facebookId', socialId: profile.id, email: profile.emails?.[0]?.value ?? '', name: profile.displayName}),
            done
        );
    };
}