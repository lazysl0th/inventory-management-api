import { Strategy, type IStrategyOptionsWithRequest, type VerifyFunctionWithRequest } from 'passport-local';
...
export default class LocalStrategy extends Passport implements IStrategy<Strategy> {
    readonly strategy: Strategy;
    private readonly options: IStrategyOptionsWithRequest;

    constructor (private readonly authService: IAuthService) {
        super();
        this.options = { usernameField: 'email', passwordField: 'password', passReqToCallback: true };
    ...
    verify: VerifyFunctionWithRequest = (req, username, password, done) => {
        ...
        this.verifyHandle<IAuthResultData>(
            async () => await this.authService.loginUserByEmail({ email: username, password: password, remember}),
            done
        );
    };
}