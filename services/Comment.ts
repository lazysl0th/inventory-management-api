...
export default class CommentService implements ICommentService {
    ...
    async createComment(content: string, inventoryId: number, userId: number): Promise<TComment> {
        const comment = await this.CommentModel.create(content, inventoryId, userId);
        this.notifier.notify(`comments:inventoryId=${comment.inventoryId}`, {comment, channel: `comments:inventoryId=${comment.inventoryId}`})
        return comment;
