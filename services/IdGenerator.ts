import { EnumCustomIdPartType, type ICustomIdFormatPart, type TCustomIdPartFormat, } from "../types/models/Inventory.js";
...
export default class IdGeneratorService implements IIdGenerator {
    ...
    private readonly partIdGenerators: Record<EnumCustomIdPartType, IPartIdGenerator>;
    ...
    constructor(
        ...
        this.partIdFormatters = new Map(partIdFormatters.flatMap(partIdformatter => partIdformatter.formats.map(format => [format, partIdformatter])
        )
    );
    }

    async _generatePartId(part: ICustomIdFormatPart, tx: Prisma.TransactionClient): Promise<string | number | bigint | Date> {
        if (part.type === EnumCustomIdPartType.TEXT && part.format) return part.format;
    ...
    _formatPartId(id: string | number | bigint | Date, format: TCustomIdPartFormat) {
        if (!format) return id;
        const idFormatter = this.partIdFormatters.get(format);
        return idFormatter ? idFormatter.format(id, format) : id;
    }

    async generateCustomId (customIdParts: ICustomIdFormatPart[], tx: Prisma.TransactionClient): Promise<string> {
        const generatedParts = await Promise.all(customIdParts.map(async(part) => {
            const id = await this._generatePartId(part, tx);
            const formatedId = this._formatPartId(id, part.format);
            return part.position === 'prefix' ? `${part.separator ?? ''}${formatedId}` : `${formatedId}${part.separator ?? ''}`;
        }));
