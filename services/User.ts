...
import type { IUserModel, TSafeUser, TSafeUserWithRoles, TUserBySafeMode, TUserCreateData, TUserUpdateData, TUserWithRoles } from "../types/models/User.js";
...
export default class UserService implements IUserService {
    ...
    async getUserById<T extends boolean = true>(id: number, safeMode: T = true as T): Promise<TUserBySafeMode<T>> {
    ...
    async getUserBySocialId(provider: TProvider, socialId: string): Promise<TSafeUserWithRoles | null> {
    ...
    async updateUser(id: number, data: TUserUpdateData): Promise<TSafeUserWithRoles> {
        return await this.UserModel.updateById(id, data);
    }

    async updateUserStatus(ids: number[], status: Status): Promise<{ count: number; }> {
        return await this.UserModel.updateStatusByIds(ids, status);
    }

    async updateUsers(ids: number[], userData: Partial<TSafeUser>): Promise<{ count: number; }> {
