...
import type { IItemData, IItemModel, IItemValue, TItem, TLike } from '../types/models/Item.js';
...
export default class ItemService implements IItemService {
    ...
    private _createValues(itemValues: Prisma.ItemValueUncheckedCreateWithoutItemInput[]): Prisma.ItemValueUncheckedCreateWithoutItemInput[] {
        return itemValues.map(value => ({
            value: typeof value.value !== 'boolean' ? value.value : value.value ? 'true' : 'false',
            fieldId: value.fieldId
        }))
    }

    private _updateValues(itemValues: IItemValue[]): Prisma.ItemValueUpdateWithWhereUniqueWithoutItemInput[] {
        return itemValues.filter(value => value.id).map(value => ({
            where: { id: value.id },
            data: {
                value: typeof value.value !== 'boolean' ? value.value : value.value ? 'true' : 'false',
    ...
    private _deleteValues(itemValues: IItemValue[]): Prisma.ItemValueScalarWhereInput {

        return { id: { notIn: itemValues.filter(value => value.id).map(value => value.id) } }
    }

    private _valuesForCreateItem(itemValues: Prisma.ItemValueUncheckedCreateWithoutItemInput[]): Prisma.ItemValueCreateNestedManyWithoutItemInput {
        return { create: this._createValues(itemValues) }
    }

    private _valuesForUpdateItem(itemValues: IItemValue[]): Pick<Prisma.ItemUpdateInput, 'values'> | undefined {
        return (itemValues.length)
            ? { values: {  
                deleteMany: this._deleteValues(itemValues),
                update: this._updateValues(itemValues),
                create: this._createValues(itemValues.filter(itemValue => !itemValue.id)),
            } }
            : undefined
    }

    async createItem(userId: number, inventoryId: number, itemValues: IItemValue[]): Promise<TItem> {
        const inventory = await this.InventoryService.getInventoryById(inventoryId);
        if (!isCustomIdFormatObject(inventory.customIdFormat)) throw new BadRequest(BAD_REQUEST.TEXT);
        ...
        return await this.ItemModel.create(itemCreateData, inventory.customIdFormat.parts);
    }
