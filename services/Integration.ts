...
import type { IDropboxApi, IUploadResultDropbox } from "../types/services/intagrations/Dropbox.js";
import type { IIntegrationService, ILocations, IUploadDataDropbox } from "../types/services/Integration.js";
import type CloudinaryApi from "./api/Cloudinary.js";
import type { IAdditionalData, IAddInfoCompositeResponse, ISalesForceApi, IGetInfoResponse } from "../types/services/intagrations/SalesForce.js";
...
export default class IntegrationService implements IIntegrationService {
    ...
    async uploadJsonToDropbox(data: IUploadDataDropbox): Promise<IUploadResultDropbox> {
    ...
    async getLocationFromSalesForce(): Promise<ILocations> {
        ...
        const countries = resonse.fields.find(field => field.name === "CountryCode");
        const states = resonse.fields.find(field => field.name === "StateCode");
        return { countries: countries?.picklistValues, states: states?.picklistValues};
    }

    async addAdditionalInfoToSalesForce(userId: number, additionalData: IAdditionalData): Promise<IAddInfoCompositeResponse> {
        const user = await this.UserService.getUserById(userId);
        if (!user) throw new NotFound(NOT_FOUND.TEXT);
        return await this.SalesForceApi.createAccountWithContact(user, additionalData);
    }

    async getAdditionalInfoFromSalesForce (userId: number): Promise<IGetInfoResponse> {
