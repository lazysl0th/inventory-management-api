...
import type { TSafeUser, TSafeUserWithRoles, TUserBySafeMode, TUserCreateData, TUserUpdateData, TUserWithRoles } from "../models/User.js";
import type { TProvider } from "./Auth.js";

export interface IUserService {
    getUserById<T extends boolean = true>(id: number, safeMode?: T): Promise<TUserBySafeMode<T>>;
    getUserByEmail(email: string): Promise<TUserWithRoles>;
    getUserBySocialId(provider: TProvider, socialId: string): Promise<TSafeUserWithRoles | null>
    ...
    updateUsers(ids: number[], userData: Partial<TSafeUser>): Promise<{ count: number; }>
