...
import type { IBodyCreateItem, IBodyItemsIds, IItemController, IParamItemId } from "../types/controllers/Item.js";
import type { IItemService } from "../types/services/Item.js";
import type { IItemData, TItem, TItemUpdateData, TLike } from "../types/models/Item.js";
...
export default class ItemController extends Controller implements IItemController {
    ...
    createItem: Handler = this.handle<IParamInventoryId, IItemData>(async (req, res) => {
        const userId = req.user.id;
        const inventoryId = req.params.inventoryId;
        const itemData = req.body.values;
        const item = await this.ItemService.createItem(userId, inventoryId, itemData)
        this.ok<TItem>(res, item);
    })

    updateItem: Handler = this.handle<IParamItemId, IItemData>(async(req, res) => {
        const itemId = req.params.itemId;
    ...
    deleteItems: Handler = this.handle<{}, IBodyItemsIds>(async(req, res) => {
