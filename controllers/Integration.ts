...
import type { IAuthCodeDropbox, IIntegrationService, ILocations, IUploadDataDropbox } from "../types/services/Integration.js";
import type { IAddInfoCompositeResponse, IAdditionalData, IGetInfoResponse } from "../types/services/intagrations/SalesForce.js";
...
export default class IntegrationController extends Controller implements IIntegrationController {
    ...
    uploadImageToCloudinary: Handler = this.handle(async(req, res) => {
        ...
        const result = await this.IntegrationService.uploadImageToCloudinary(req.file);
        this.ok<UploadApiResponse>(res, result);
    });

    uploadJsonToDropbox: Handler = this.handle<{}, IUploadDataDropbox>(async(req, res) => {
        const uploadData = req.body
        const result = await this.IntegrationService.uploadJsonToDropbox(uploadData);
        this.ok<IUploadResultDropbox>(res, result);
    });

    exchangeDropBoxCodeOnToken: Handler = this.handle<{}, {}, IAuthCodeDropbox>(async(req, res) => {
    ...
    getLocationFromSalesForce: Handler = this.handle(async(req, res) => {
        const location = await this.IntegrationService.getLocationFromSalesForce();
        this.ok<ILocations>(res, location)
    });

    addAdditionalInfoToSalesForce: Handler = this.handle<IParamUserId, IAdditionalData>(async (req, res) => {
        ...
        const result = await this.IntegrationService.addAdditionalInfoToSalesForce(userId, additionalData);
        this.ok<IAddInfoCompositeResponse>(res, result)
    });

    getAdditionalInfoFromSalesForce: Handler = this.handle<IParamUserId>(async (req, res) => {
        const userId = req.params.userId;
        const additionalInfo = await this.IntegrationService.getAdditionalInfoFromSalesForce(userId);
