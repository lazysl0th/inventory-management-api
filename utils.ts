...
import type { IAccessTokenData, IResetPasswordTokenData } from './types/services/Auth.js';
import type { TClientData } from './types/services/Ws.js';

export function replaceParamsInTemplate(params: Record<string, string>, template: string): string {
    return Object.entries(params).reduce((result, [params, paramsValue]) => result.replaceAll(`{${params}}`, paramsValue), template)
}

export function isCustomIdFormatObject(obj: unknown): obj is ICustomIdFormat {
    if (typeof obj !== 'object' || obj === null) return false;
    const { parts, summary } = obj as any;
...
export function isResetPasswordTokenData(obj: unknown): obj is IResetPasswordTokenData {
...
export function isAccessTokenData(obj: unknown): obj is IAccessTokenData {
    return isObject(obj) && 'type' in obj && (obj.type === 'access' || obj.type === 'refresh');
}
