...
import type { IInventoryModel, TInventory, TInventoryCreateData, TInventoryUpdateData } from '../types/models/Inventory.js';
...
export default class InventoryModel extends Model<TInventory, TInventoryCreateData, TInventoryUpdateData> implements IInventoryModel {
    ...
    async getAll(sortOrder?: EnumInventorySortOrder, ownerId?: number, allowedUserId?: number, isPublic?: boolean): Promise<TInventory[]> {
        return await prisma.inventory.findMany({
            ...(ownerId && { where: { ownerId } }),
            ...(allowedUserId && isPublic && { where: { OR: [ { allowedUsers: { some: { id: allowedUserId } } }, { isPublic } ] } }),
            select: this.inventorySelect,
    ...
    async updateById(id: number, data: TInventoryUpdateData, version: number): Promise<TInventory> {
