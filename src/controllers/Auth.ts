  ...
  implements IAuthController
{
  ...
  registerUser: Handler = this.handle<object, IRegData | ISocialAuthData>(
    async (req, res) => {
      ...
      this.created<{ accessToken: string }>(res, {
        accessToken: authData.authTokens.accessToken,
      });
    },
  );
  ...
  loginUserByEmail: Handler = this.handle(async (req, res) => {
    ...
    this.ok<ISafeUserWithToken>(res, {
      user,
  ...
  resetUserPassword: Handler = this.handle<object, TResetUserPasswordData>(
    async (req, res) => {
  ...
  changeUserPassword: Handler = this.handle<object, IChangePasswordData>(
    async (req, res) => {
