...
export default class WsService implements IWsService {
  ...
  private init() {
    this.wsServer.on("connection", (ws: AliveWebSocket) =>
  ...
  registerHandlers(messageHandlers: IMessageHandler[]): void {
    messageHandlers.forEach((messageHandler) =>
      this.messageHandlers.set(messageHandler.type, messageHandler),
    );
  }

  private handleConnection(ws: AliveWebSocket) {
    ws.isAlive = true;
    this.clients.add(ws);
