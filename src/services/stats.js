const getNumStats = (fields, items) =>
  fields.map(({ title }) => {
    const values = items
      .map((item) =>
        Number(item.values.find((v) => v.field.title === title)?.value),
      )
      .filter((value) => !isNaN(value));
    if (!values.length)
      return { field: title, average: null, min: null, max: null };
...
const getTextStats = (fields, items) =>
  fields.map(({ title }) => {
    const counts = items
      .map(
        (item) =>
          item.values.find((value) => value.field.title === title)?.value,
      )
      .filter(Boolean)
      .reduce((acc, value) => {
