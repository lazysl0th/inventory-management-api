...
import type {
  DoneCallbackWithOptions,
  IVerifyOptionsExtends,
  TAuthResult,
  VerifyUserHandler,
...
export default abstract class Passport {
  protected async verifyHandle<TUser>(
    handler: VerifyUserHandler<TUser>,
  ...
  private static _createAuthError(info: IVerifyOptionsExtends): IError | void {
    switch (info.message) {
      case UNAUTHORIZED.TEXT ||
        ...
        return new Unauthorized(UNAUTHORIZED.TEXT);
      case BLOCKED.TEXT:
        return new Forbidden(BLOCKED.TEXT);
      case NOT_FOUND.TEXT:
        return new NotFound(NOT_FOUND.TEXT);
      default:
        return new Unauthorized(info.message || UNAUTHORIZED.TEXT);
    }
  }

  private static _handleAuthenticate(
    resolve: (authResult: TAuthResult) => void,
